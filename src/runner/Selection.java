...
public Selection (ArrayList <Attribute> attsInInputFile,
    ...
    try {            
      ...
      String cmdarr = compiler+ " -O3 -o " + cppDir + "a.out -Wno-write-strings "+cppDir+"Selection.cc";//System.out.println(cmdarr);
    ...
    try {            
      Runtime rt = Runtime.getRuntime();
      String cmdarr= cppDir + "a.out "+"tables/"+inFile+" "+"tables/"+outFile;
      Process proc = rt.exec (cmdarr);
      int exitVal = proc.waitFor();
      if (exitVal != 0) {
    	  System.out.println("Selection.cmdarr: "+cmdarr+"\n");
