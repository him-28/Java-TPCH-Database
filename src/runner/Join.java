...
public Join (ArrayList <Attribute> attsInLeftInputFile,
    ...
    try {            
      ...
      String cmdarr = compiler+ " -O3 -o " + cppDir + "a.out -Wno-write-strings "+ cppDir + "Join.cc";//System.out.println(cmdarr);
    ...
    try {            
      Runtime rt = Runtime.getRuntime();
      String cmdarr = cppDir + "a.out "+"tables/"+inFileLeft+" "+ "tables/"+inFileRight+" "+"tables/"+outFile;
      Process proc = rt.exec (cmdarr);
      int exitVal = proc.waitFor();
      if (exitVal != 0) {
    	  System.out.println("Join.cmdarr: "+cmdarr+"\n");
