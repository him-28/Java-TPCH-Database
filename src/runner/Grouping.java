...
public Grouping (ArrayList <Attribute> attsInInputFile,
    ...
    try {            
      ...
      String cmdarr = compiler+ " -O3 -o " + cppDir + "a.out -Wno-write-strings "+ cppDir + "GroupBy.cc";//System.out.println(cmdarr);
    ...
    try {            
      Runtime rt = Runtime.getRuntime();
      String cmdarr = cppDir + "a.out "+"tables/"+inFile+" "+"tables/"+outFile;//System.out.println(cmdarr);
      Process proc = rt.exec (cmdarr);
      int exitVal = proc.waitFor();
      if (exitVal != 0) {
    	  System.out.println("Grouping.cmdarr: "+cmdarr+"\n");
